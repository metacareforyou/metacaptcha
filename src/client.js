  ...
  const doneCallback = useCallback(async () => {
    if (!hasSolved.current && challengeData.current.ecChallenge) {
      try {
        ...
        setCaptchaStatus('Solving RSA Challenge...');

        // Retrieve RSA components from server's challenge
        const { encryptedMessage, n, e } = challengeData.current.rsaChallenge;

        // Create the RSAKey object using `jsrsasign`
        const rsaKey = new RSAKey();
        rsaKey.setPublic(n, e); // Use modulus `n` and public exponent `e` for RSA operations

        // Decrypt the RSA message using the raw RSA algorithm
        const decryptedHex = rsaKey.decrypt(b64tohex(encryptedMessage));
        const decryptedMessage = decryptedHex; // If further decoding is needed, adjust accordingly

        setCaptchaStatus('Verifying solution...');
        
        // Step 3: Verify the EC and RSA solutions with the backend
